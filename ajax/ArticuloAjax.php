		...
		case 'SaveOrUpdate':			
			...
			if(move_uploaded_file($imagen, "../Files/Articulo/".$ruta)){
				...
				if(empty($_POST["txtIdArticulo"])){
					...
					if($objArticulo->Registrar($idcategoria, $idunidad_medida, $nombre,$codigo, $descripcion, "Files/Articulo/".$ruta)){
					...
					if($objArticulo->Modificar($idarticulo, $idcategoria, $idunidad_medida, $nombre,$codigo, $descripcion, "Files/Articulo/".$ruta)){
				...
				if(empty($_POST["txtIdArticulo"])){
					...
					if($objArticulo->Registrar($idcategoria, $idunidad_medida, $nombre,$codigo, $descripcion, $ruta_img)){
						echo "Articulo Registrado";
					}else{
						echo "Articulo no ha podido ser registado.";
					}
					...
					$idarticulo = $_POST["txtIdArticulo"];
					if($objArticulo->Modificar($idarticulo, $idcategoria, $idunidad_medida, $nombre,$codigo, $descripcion, $ruta_img)){
						echo "Informacion del Articulo ha sido actualizada";
					}else{
						echo "Informacion del Articulo no ha podido ser actualizada.";
					}
            ...
            $i = 1;
     		while ($reg = $query_Tipo->fetch_object()) {
     			...
     			$data[] = array("id"=>$i,
					...
					'<button class="btn btn-warning" data-toggle="tooltip" title="Editar" onclick="cargarDataArticulo('.$reg->idarticulo.',\''.$reg->idcategoria.'\',\''.$reg->idunidad_medida.'\',\''.$reg->nombre.'\',\''.$reg->codigo.'\',\''.$reg->descripcion.'\',\''.$reg->imagen.'\')"><i class="fa fa-pencil"></i> </button>&nbsp;'.
		...
		case "listArtElegir":
            ...
            $i = 1;
     		while ($reg = $query_Tipo->fetch_object()) {
     			...
     			$data[] = array(
     				"0"=>'<button type="button" class="btn btn-warning" data-toggle="tooltip" title="Agregar al detalle" onclick="Agregar('.$reg->idarticulo.',\''.$reg->nombre.'\',\''.$reg->codigo.'\')" name="optArtBusqueda[]" data-nombre="'.$reg->nombre.'" id="'.$reg->idarticulo.'" value="'.$reg->idarticulo.'" ><i class="fa fa-check" ></i> </button>',
     				"1"=>$i,
					"2"=>$reg->categoria,
					"3"=>$reg->unidadMedida,
					"4"=>$reg->nombre,
